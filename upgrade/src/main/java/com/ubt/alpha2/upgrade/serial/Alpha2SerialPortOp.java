...
import static com.ubt.alpha2.upgrade.serial.SerialCommandExecutor.DEFAULT_COMMAND_SID;
import static com.ubt.alpha2.upgrade.serial.SerialCommandExecutor.MAX_INDEX;
 ...
 */
 class Alpha2SerialPortOp implements ISerialPortOpProxy {
    ...
    public void sendCommand(byte sessionId, byte cmd, byte index, byte[] data, ISerialCommandCallback callback) {
        ...
        Timber.e("sendCommand: cmd=%s,index=%d", ConvertUtils.byte2HexString(cmd), index);
        ...
        try {
            ...
            if (cmd == ChestCmds.CMD_SEND_DATA_TO_MOTOR && serialType == SerialConstants.TYPE_CHEST){
                ...
                semaphore.tryAcquire(1, 50, TimeUnit.SECONDS);
            }
            LogUtils.d("time: "+(System.currentTimeMillis()-time1));
    ...
    private void handleReadRequest() {
        synchronized (readLock){
            ...
            try {
                ...
                if (read > 0){
                    mBBL.add(buffer);
//                byte[] packetBytes = Alpha2SerialProtocol.findValidPacket(mBBL);
//                if (packetBytes != null){
...
//                }
                    ...
                    while((packetBytes = Alpha2SerialProtocol.findValidPacket(mBBL)) != null){
                        ...
                        //Timber.d("remain: "+ mBBL.remaining()+ " peek: "+ConvertUtils.bytes2HexString(mBBL.peekBytes(mBBL.remaining())));
                    }
    ...
    private void handlePacket(Alpha2SerialProtocol answer) {
        //LogUtils.d("handlePacket_index:"+ answer.getIndex());
            ...
            if (cmd < 0x80 && answer.getId() == DEFAULT_COMMAND_SID) {
                ...
                synchronized (this) {
                    ...
                    if (param == null){
                        mAnswerMaps.put(request, null);
                        final Semaphore semaphore = mRequstMaps.get(request);
                        LogUtils.d("notify send thread param == null");
                        if (semaphore != null)
                            semaphore.release();//通知发送线程
                        return;
                    }
