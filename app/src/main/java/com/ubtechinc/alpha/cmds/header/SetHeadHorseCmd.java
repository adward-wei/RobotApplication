...
public class SetHeadHorseCmd extends HeadCmds {
    ...
    public SetHeadHorseCmd(SerialCommandReceiver receiver, int color, int bright, int rled, int lled, int on, int total) {
        super(receiver, null);
        this.color = color;
        this.bright = bright;
        this.rled = rled;
    ...
    public void execute() {
        if (SysUtils.is5Mic()){
            LedControl.open();
            boolean ret = LedControl.ledSetHead(color,bright,rled,lled,on,0,total, 0x03);
