...
public class SetHeadFlashCmd extends HeadCmds {
    ...
    public SetHeadFlashCmd(SerialCommandReceiver receiver, int color, int bright, int rled, int lled, int on, int total) {
        super(receiver, null);
        this.color = color;
        this.bright = bright;
        this.rled = rled;
    ...
    public void execute() {
        if (SysUtils.is5Mic()){
            LedControl.open();
            boolean ret = LedControl.ledSetHead(color,bright,rled,lled,on,(color > LedColor.BLUE.value?  0: 1000),total,(color > LedColor.BLUE.value?  0x05: 0x00));
