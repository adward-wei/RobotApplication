...
import com.ubtechinc.alpha.jni.SerialPortFile;
...
class Alpha2SerialPortUtil implements ISerialPortUtil {
    ...
    public void open() throws IOException, SecurityException {
        if (file == null) {
            this.file = new SerialPortFile(new File(mFileName), mBandrate, 0);
        }
    }
...
//                Timber.d("read bytes:%s",ConvertUtils.bytes2HexString(bytes));
                if (read <= 0) return 0;
                in.put(bytes);
                in.position(0);
                in.limit(read);
